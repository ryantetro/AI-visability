...
function DomainFavicon({ domain, isOwn, isComp }: { domain: string; isOwn: boolean; isComp: boolean }) {
  ...
  if (failed) {
    return (
      <Globe className={cn(
        'h-4 w-4 shrink-0',
        isOwn ? 'text-[#25c972]' : isComp ? 'text-[#ff8a1e]' : 'text-zinc-500'
      )} />
    );
  }
  return (
...
function CitationChartTooltip({ active, payload, label }: {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-zinc-900/95 px-4 py-3 shadow-2xl backdrop-blur-sm min-w-[170px]">
      <p className="text-[13px] font-semibold text-zinc-100 mb-2.5 pb-2 border-b border-white/8">{label}</p>
      <div className="space-y-2">
        {sorted.map((p) => (
          <div key={p.dataKey} className="flex items-center gap-2.5">
            <EngineIcon engine={p.dataKey} className="size-5" />
            <span className="text-[12px] text-zinc-300 flex-1">{getAIEngineLabel(p.dataKey as never)}</span>
            <span className="text-[14px] font-bold tabular-nums text-white">{p.value}</span>
          </div>
        ))}
      </div>
    </div>
...
export function CitationTrackingPanel({ report }: { report: DashboardReportData }) {
      ...
      <DashboardPanel className="p-8">
        <div className="flex flex-col items-center text-center py-8">
          <div className="mb-4 rounded-full bg-white/[0.04] p-4">
            <Link2Off className="h-8 w-8 text-zinc-500" />
          </div>
          <h3 className="text-[15px] font-semibold text-zinc-200">No citations found</h3>
          ...
          <div className="mt-5 rounded-xl border border-white/8 bg-white/[0.02] px-4 py-3 text-left max-w-sm">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-zinc-500 mb-2">How to improve</p>
            <ul className="space-y-1.5 text-[12px] text-zinc-400">
              <li>Publish authoritative, well-structured content</li>
              <li>Add an <code className="text-zinc-300 bg-white/[0.06] px-1 rounded text-[11px]">llms.txt</code> file to your site</li>
  ...
  return (
    <div className="space-y-6">
      ...
      <DashboardPanel className="p-5">
        <p className="mb-4 text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500">Cited Domains</p>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-[12px]">
            <thead>
              <tr className="border-b border-white/8 text-[10px] font-semibold uppercase tracking-[0.15em] text-zinc-500">
            ...
            <tbody>
              {sortedDomains.slice(0, 15).map(([domainName, info]) => {
                ...
                return (
                  <tr key={domainName} className="border-b border-white/5">
                    {/* Domain column */}
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-[13px] text-zinc-200 truncate">{domainName}</span>
                          ...
                          <span className="inline-block mt-0.5 rounded-full bg-white/[0.06] px-2 py-0.5 text-[10px] text-zinc-500">
                    ...
                    <td className="py-3.5 pr-3 text-right w-[160px]">
                      <div className="flex items-center justify-end gap-2">
                        <span className="text-[13px] font-bold tabular-nums text-zinc-200">{info.count}</span>
                        <span className="text-[11px] tabular-nums text-zinc-500">{citePct}%</span>
                      </div>
                    ...
                    <td className="py-3.5 text-right w-[160px]">
                      <div className="flex items-center justify-end gap-2">
                        <span className="text-[13px] font-bold tabular-nums text-zinc-200">{info.brandCites}</span>
                        <span className="text-[11px] tabular-nums text-zinc-500">{brandCitePct}%</span>
                      </div>
                      <div className="mt-1.5 h-1 w-full rounded-full bg-white/5">
                        <div
        ...
        <DashboardPanel className="p-6">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="group relative mt-1 shrink-0">
              <Info className="h-4 w-4 cursor-help text-zinc-600 transition-colors group-hover:text-zinc-400" />
              <div className="pointer-events-none absolute right-0 top-full z-20 mt-2 w-[280px] rounded-xl border border-white/10 bg-zinc-900/95 px-4 py-3 opacity-0 shadow-2xl backdrop-blur-sm transition-opacity group-hover:pointer-events-auto group-hover:opacity-100">
                <p className="text-[11px] font-semibold text-zinc-200">Why only some engines?</p>
          ...
          <div className="mt-6 flex gap-0">
            ...
            <div className="hidden lg:flex w-[200px] shrink-0 flex-col border-l border-white/[0.06] pl-6 ml-6">
                ...
                {engineCounts.map((ec, i) => {
                  ...
                  return (
                    <div key={ec.engine} className="flex items-start gap-3">
                      <span className="mt-0.5 text-[13px] font-bold tabular-nums text-zinc-600">{i + 1}</span>
                      <EngineIcon engine={ec.engine} className="mt-0.5 size-5 shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-[12px] font-semibold capitalize text-zinc-200">
                          {getAIEngineLabel(ec.engine as never)}
                        </p>
                        <div className="flex items-baseline gap-1.5">
                          <span className="text-[15px] font-bold tabular-nums text-white">{ec.count}</span>
