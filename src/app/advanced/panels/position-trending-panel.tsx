      ...
      <div className="mt-5 h-[220px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
            ...
            <YAxis reversed domain={[1, 'auto']} tick={{ fontSize: 10, fill: '#6b7280' }} axisLine={{ stroke: '#e5e7eb' }} tickLine={false} label={{ value: 'Position', angle: -90, position: 'insideLeft', style: { fontSize: 9, fill: '#6b7280' } }} />
            <Tooltip contentStyle={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 11 }} labelStyle={{ color: '#6b7280' }} />
            {engines.map((engine) => (
      ...
      <div className="mt-3 flex flex-wrap justify-center gap-4">
        {engines.map((engine) => (
          <div key={engine} className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-gray-500">{getAIEngineLabel(engine as TrendPoint['engine'])}</span>
          </div>
        ))}
      </div>

      <div className="mt-4">
        <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-gray-500">Weekly mention rate</p>
          ...
          {engines.map((engine) => {
            ...
            return (
              <div key={engine} className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-center">
                <p className="text-[10px] text-gray-500">{getAIEngineLabel(engine as TrendPoint['engine'])}</p>
                <p className="text-sm font-bold tabular-nums text-gray-700">{latest?.mentionRate ?? 0}%</p>
