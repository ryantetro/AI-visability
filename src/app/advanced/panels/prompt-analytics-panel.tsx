    ...
    return (
      <DashboardPanel className="p-6">
        ...
        <div className="mt-6 flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 bg-white/[0.02] py-12">
          ...
          <p className="mt-3 text-sm font-medium text-zinc-400">Upgrade to Starter to unlock prompt analytics</p>
          <p className="mt-1 text-[12px] text-zinc-600">See weekly mention rates, engine comparisons, and trend analysis.</p>
        </div>
      </DashboardPanel>
    );
  }
    ...
    <DashboardPanel className="p-6">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-xl border border-white/8 bg-white/[0.03] px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-zinc-500">Total Checks</p>
          <p className="mt-1 text-2xl font-bold text-white">{totalChecks.toLocaleString()}</p>
        </div>
        <div className="rounded-xl border border-white/8 bg-white/[0.03] px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-zinc-500">Mention Rate</p>
          <p className={cn('mt-1 text-2xl font-bold', overallRate >= 50 ? 'text-[#25c972]' : overallRate >= 25 ? 'text-[#ffbb00]' : 'text-[#ff5252]')}>
            {overallRate}%
          </p>
        </div>
        <div className="rounded-xl border border-white/8 bg-white/[0.03] px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-zinc-500">Best Engine</p>
          <div className="mt-1 flex items-center gap-2">
            {bestEngine && (
              <>
                <EngineIcon engine={bestEngine.engine} className="size-5" style={{ color: ENGINE_COLORS[bestEngine.engine] ?? '#71717a' }} />
                <span className="text-lg font-bold text-white">{bestEngine.rate}%</span>
              </>
            )}
          </div>
        </div>
        <div className="rounded-xl border border-white/8 bg-white/[0.03] px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-zinc-500">Weekly Trend</p>
          <div className="mt-1 flex items-center gap-1.5">
            ...
            <span className={cn(
              'text-2xl font-bold',
              weeklyDelta > 0 ? 'text-[#25c972]' : weeklyDelta < 0 ? 'text-[#ff5252]' : 'text-zinc-400',
            )}>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_320px]">
        ...
        <div className="rounded-xl border border-white/8 bg-white/[0.02] p-4">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.15em] text-zinc-500">Mention Rate Over Time</p>
              ...
              <LineChart data={chartData} margin={{ top: 8, right: 16, bottom: 8, left: 4 }}>
                ...
                {activeEngines.map(engine => (
                  <Line
                    ...
                    strokeWidth={2}
                    dot={{ r: 3, fill: '#0a0a0c', stroke: ENGINE_COLORS[engine] ?? '#71717a', strokeWidth: 2 }}
                    activeDot={{ r: 5, fill: ENGINE_COLORS[engine] ?? '#71717a', stroke: '#0a0a0c', strokeWidth: 2 }}
          ...
          <div className="-mt-1 flex items-center justify-center gap-5">
            {activeEngines.map(engine => (
              <div key={engine} className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-medium text-zinc-400">{getAIEngineLabel(engine)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-white/8 bg-white/[0.02] p-4">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.15em] text-zinc-500">Engine Comparison</p>
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barData} layout="vertical" margin={{ left: 0, right: 16, top: 8, bottom: 8 }}>
                <XAxis type="number" domain={[0, 100]} tick={{ fill: '#71717a', fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={(v: number) => `${v}%`} />
                <YAxis type="category" dataKey="name" width={85} tick={{ fill: '#a1a1aa', fontSize: 11 }} axisLine={false} tickLine={false} />
                <Tooltip
                  content={({ active, payload }) => {
                    ...
                    return (
                      <div className="rounded-lg border border-white/10 bg-zinc-900 px-3 py-2 text-xs shadow-lg">
                        ...
                        <span className="ml-2 font-bold text-white">{item.rate}%</span>
                      </div>
                    );
                  }}
          ...
          <div className="mt-3 space-y-2">
            {engineStats.slice(0, 4).map(e => (
              <div key={e.engine} className="flex items-center gap-2">
                ...
                <span className="flex-1 text-[11px] text-zinc-400">{getAIEngineLabel(e.engine)}</span>
                <span className="text-[11px] font-medium text-zinc-300">{e.rate}%</span>
                {e.delta !== 0 && (
...
function TrendTooltip({
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-zinc-900 px-3 py-2.5 shadow-lg">
      <p className="text-[11px] font-medium text-zinc-400 mb-1.5">Week of {label}</p>
      {payload
        ...
        .map(p => (
          <div key={p.dataKey} className="flex items-center gap-2 py-0.5">
            ...
            <span className="text-[11px] text-zinc-300">{getAIEngineLabel(p.dataKey as AIEngine)}</span>
