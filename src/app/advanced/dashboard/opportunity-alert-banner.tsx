...
function OpportunityRing({ crawls, referrals }: { crawls: number; referrals: number }) {
  ...
  return (
    <div className="relative flex items-center justify-center">
      <svg width="120" height="120" viewBox="0 0 100 100" className="drop-shadow-[0_0_24px_rgba(255,138,30,0.1)]">
        {/* Background track */}
    ...
    <div className="relative overflow-hidden rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50 shadow-sm">
      <div className="relative px-6 pb-6 pt-5">
        ...
        <div className="flex items-center justify-between">
          <span className="inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-100/60 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-amber-800">
        ...
        <div className="mt-6 flex flex-col gap-4 rounded-xl border border-gray-200 bg-white/70 p-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="mt-2.5 flex flex-wrap gap-2">
              {opportunity.topProviders.map((provider) => (
                <div
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-gray-50 px-3 py-1.5 transition-colors hover:bg-gray-100"
                >
                  <EngineIcon engine={provider.provider} className="size-3.5" />
                  <span className="text-[11px] font-medium text-gray-700">
                    {PROVIDER_LABELS[provider.provider] ?? provider.provider}
                  </span>
                  <span className="rounded-md bg-gray-200 px-1.5 py-0.5 text-[10px] font-bold tabular-nums text-gray-900">
                    {provider.visits}
                  </span>
                </div>
              ))}
          ...
          <div className="flex shrink-0 items-center gap-2.5">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-b from-[#ff9a3e] to-[#ff8a1e] px-5 py-2.5 text-[12px] font-semibold text-white shadow-sm transition-all hover:shadow-md hover:brightness-105"
            >
              Open Priority Fixes
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-[12px] font-medium text-gray-700 transition-colors hover:border-gray-400 hover:bg-gray-50"
            >
              See traffic details
            </button>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="mt-2.5 grid gap-2 md:grid-cols-3">
              {opportunity.topPages.map((page, index) => (
                <div
                  key={page.path}
                  className="group rounded-xl border border-gray-200 bg-white/70 p-3.5 transition-colors hover:border-gray-300 hover:bg-white"
                >
                  <div className="flex items-center justify-between">
